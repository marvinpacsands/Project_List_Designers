...
function init() {
  ...
  fetch('/api/bootstrap?email=' + encodeURIComponent(email))
    .then(r => r.json())
    .then(b => {
      BOOT = b;
      ...
      // If NOT purely PM or Ops, show Mine (e.g. Designer)
      // But we handled PM case above.
      // So checks below are primarily for Pure Designers or weird combos.
      if (!availableTabs.includes('mine')) {
      ...
      // Default Tab Logic
      // PMs default to PM view (requested)
...
function autoRefresh() {
  ...
  if (ACTIVE_TAB === 'mine') loadMine();
  else if (ACTIVE_TAB === 'pm') loadPM(document.getElementById('pmSelect').value);
  else if (ACTIVE_TAB === 'ops') loadOperational();
}
