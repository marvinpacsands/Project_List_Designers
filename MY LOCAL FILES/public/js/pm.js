...
function loadPM(pm) {
  // Impersonation Support: URL email overrides session logic
  ...
  // Note: server.js uses 'email' param to identify the USER. 
  // If impersonating, we pass the impersonated user's email.
  // 'pmName' param further filters the list. 

  const inc = document.getElementById('includeUnassigned').checked;
  const pmQuery = pm || ''; // explicit filter or empty

  if (pmQuery !== 'Unassigned') {
    window.STICKY_UNASSIGNED = [];
  }

  fetch('/api/projects?mode=pm&email=' + encodeURIComponent(emailParam) + '&pmName=' + encodeURIComponent(pmQuery) + '&includeUnassigned=' + inc + '&_t=' + Date.now())
    .then(r => r.json())
    .then(res => {
      // res: { projects, pmList, statusList, people, totalUnassigned, designerCounts }
      window.GLOBAL_META = res; // Expose globally for debugging/usage
      ...
      // Populate PM Dropdown with explicit options
      ...
      if (pmSel) {
        ...
        // Explicit Unassigned Option if not present
...
function showDesignerTooltip(e, email) {
  // Try to find email from name if passed as name
  // Actually curName is often a Name. API needs Email?
  // API uses robust check (Name OR Email). So Name works! 
  // But wait, if multiple people have same name? Assuming unique for now.
  // Ideally lookup email from PEOPLE_LIST.
  ...
  if (DESIGNER_CACHE[query]) {
    ...
    // Fetch
    fetch('/api/projects?mode=mine&email=' + encodeURIComponent(query))
      .then(r => r.json())
      .then(res => {
        // Map to simple list: { priority, projectNumber, projectName }
        const list = (res.projects || []).map(p => ({
          priority: p.my?.priority || '',
          num: p.projectNumber,
          name: p.projectName
        })).filter(x => x.priority && !isNaN(parseInt(x.priority))); // Only ranked items

        // Sort by priority
        list.sort((a, b) => parseInt(a.priority) - parseInt(b.priority));
...
async function savePM(idx, card) {
  // Find row in either list to check permissions/existence
  ...
  // Gather Data
  const pmAssigned = card.querySelector('.form-grid select').value; // PM Assign Dropdown
  ...
  // Optimistic Update: Update ALL instances (Main List + Sticky List) to keep UI in sync
  const pRow = PM_ROWS.find(r => r.rowIndex === idx);
  const sRow = (window.STICKY_UNASSIGNED || []).find(r => r.rowIndex === idx);
  const targets = [pRow, sRow].filter(x => x); // Filter undefined
  const uniqueTargets = [...new Set(targets)]; // Dedupe if same object logic

  uniqueTargets.forEach(target => {
    if (target.pm) {
      target.pm.notes = pmNote;
    }
    target.pmName = pmAssigned;

    // Update team array locally
  ...
  // STICKY FIX: If we are in "Unassigned" view, keep this row visible even if assigned
  const pmSel = document.getElementById('pmSelect');
  ...
  fetch('/api/update', {
    ...
    if (msg) msg.textContent = 'Upd ' + res.savedAtDisplay;
...
function saveCustomOrder(orderedRowIndexes) {
  ...
  // Update GLOBAL_META immediately (before fetch) so render() sees new order
  ...
  fetch('/api/custom-order', {
    ...
    body: JSON.stringify({
      email: emailParam,
      pmName: pmName,
      orderedRowIndexes: orderedRowIndexes
    })
  }).then(r => r.json()).then(res => {
    console.log('Custom order saved:', res);
  }).catch(e => {
...
async function executeRemoteShift(pmRowIndex, desSlot, desName, newPrio) {
  // 1. Get Designer's email/identifier
  ...
  try {
    // 2. Fetch their current list
    const r = await fetch('/api/projects?mode=mine&email=' + encodeURIComponent(query));
    const data = await r.json();
    let projects = data.projects || [];

    // 3. Locate target project in their list
    ...
    const updates = [];
    const targetPrioInt = parseInt(newPrio);
    ...
    // 4. Cascade/Bump Logic (Same as mine.js executeShift)
    // We are inserting at 'targetPrioInt'.
    // If 'targetPrioInt' is occuped by SOMEONE ELSE, move them to +1.
    // Repeat.

    // Get all ranked items (excluding target)
    let others = projects.filter(p => p.rowIndex !== pmRowIndex);

    // Create a map or lookup for chain
    // We only care about items that are "in the way".
    // "In the way" means priority == bumpPrio.
    ...
    while (true) {
      if (bumpVal > 10) {
        // Bumped off the list
        // Find who had 10? No, we check who HAS 'bumpVal'.
        // If bumpVal is 11, and someone has 11... wait.
        // If someone HAD 10, and we bump them to 11.
        // We should just unassign or keep as 11 (if system allows >10).
        // User requirement: "must have exactly 10 prioritized".
        // Usually 1-10 is the range.
        // If we bump to 11, let's effectively "unprioritize" it (set to '').

        // Wait, previous logic was strict 1-10 dropdown? yes.
        // Unprioritize is safer for compliance count logic.
        // Let's find the item at 10 (which is now moving to 11)
        // Actually, in previous iteration of loop, bumpVal was 10.
        // We found someone at 10. We move them to 11.
        // So we are looking for someone at 10.
        // ...

        // Loop logic:
        // Check if anyone has 'bumpVal'.
        // If yes -> Move them to 'bumpVal + 1'.
        // Continue loop with 'bumpVal + 1'.

        // If bumpVal > 10, we stop?
        // If we bump 10 -> 11. We set priority to ''.
        // But we need to identify the row.
        break;
      }

      // Find conflict at bumpVal
      const conflict = others.find(p => parseInt(p.my?.priority) === bumpVal);

      if (!conflict) {
        // Slot is free! Chain ends.
        break;
      }

      // Move conflict to next
      const nextVal = bumpVal + 1;

      let newStr = String(nextVal);
      if (nextVal > 10) newStr = ''; // Unassign if bumped off 10
      ...
      if (newStr === '') {
        // If we kicked someone out to null, the chain definitely ends here
        break;
      }
